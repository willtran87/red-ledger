...
The selected generated sources live under `art/source/keyed/`, their extracted copies under `art/source/alpha/`, and normalization authorities under each directory's `normalized/` child. `tools/validate_art_library.py` registers all three sheets at eight cells each.
...
Total source sheets: 3. Total runtime seeds: 24. The catalog generator requires no schema change: each directory becomes one `effects` family with eight parsed frames. Add all 24 records to `manifests/effect-runtime-metadata.json` with `size: [32, 32]` so catalog references retain dimensions.
...
Repeat slicing for `world-feedback` and `death-feedback`. The slicer's files are working crops only. Finalize each numbered crop with the fixed center pivot; example for frame 01:
...
After all 24 files and metadata records exist:
...
The generator writes the synchronized catalogs to `assets/data/runtime-assets.json`, `docs/data/runtime-assets.json`, and `game/public/data/runtime-assets.json`. Update the release completeness assertion in `implementation/generate-runtime-catalog.mjs` from `3501` to `3525` PNGs after the files land. This batch adds exactly 24 runtime PNGs.
...
- All 24 runtime PNGs are RGBA, `32x32`, palette-locked, and have transparent corners.
...
- Catalog `effects` contains three new families and 24 new frames after regeneration.
