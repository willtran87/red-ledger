...
await page.locator('#save-slot-list .slot-row button').first().click();
await page.click('#load-game');
assert((await page.locator('#load-slot-list .slot-row button').first().isEnabled()), 'Written manual slot is not loadable');
await page.screenshot({ path: 'output/combat-save/load-slots.png' });
await page.locator('#load-slot-list .slot-row button').first().click();
