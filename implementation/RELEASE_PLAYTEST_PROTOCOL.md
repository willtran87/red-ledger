...
- Previous automated software candidate: `e708749138333697a187be091485461f56ec246e` (`e708749`)
- Candidate under review: `2ed9f0f804653c42a26cb4ebfeba9ae84f36f1a0` (`2ed9f0f`)
- Automated preflight and exact-candidate live deployment: **Pass**; exact output, structured evidence, committed-file hashes, and Pages run `29882893317` are archived under `manifests/`
...
The current candidate includes the later combat-hit, contextual route-hint, grounded-defeat, and chroma-fringe fixes, the corrected runtime asset catalog, a materially clearer first-run orientation, and a stable title layout when its meaningful image decodes late. Ordinary desktop briefing copy uses an 18px map title, 13px objective and bindings, 12px metadata, and 11px action labels, with stronger 2560 scaling and preserved compact/touch containment. It passed a clean `npm ci`, 397 tests across 36 files, the nested production package, 36 registered browser scenarios, Chromium/Firefox/WebKit smoke, exact 3,660-file `dist`/Pages parity, a zero-vulnerability production audit, 11 focused public/audio tests across three files, a current 3,547-PNG runtime catalog, and the complete alpha-boundary/chroma/spacing validator. The delayed-image contract holds title and action geometry within 0.5px at desktop, high-resolution, portrait, and landscape breakpoints. GitHub Pages then deployed the committed 3,661-file tree including `.nojekyll`; seven sampled live HTML, JS, CSS, audio, music, and particle-art byte streams match that tree. The installed browser-game client reaches the title and live E1M1 gameplay locally and publicly with complete assets, ready authored audio during gameplay, matching state, and no captured page errors. Use `manifests/release-candidate-evidence.json`, `manifests/release-candidate-preflight.txt`, and `manifests/pages-artifact-sha256.txt` as the candidate header evidence for every human record below.
