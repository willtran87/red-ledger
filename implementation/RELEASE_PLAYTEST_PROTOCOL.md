...
- Previous automated software candidate: `2ed9f0f804653c42a26cb4ebfeba9ae84f36f1a0` (`2ed9f0f`)
- Candidate under review: `e64ff6e14db39680691e12cf1a2e50118ef04c95` (`e64ff6e`)
- Automated preflight and exact-candidate live deployment: **Pass**; exact output, structured evidence, committed-file hashes, and Pages run `29884808058` are archived under `manifests/`
...
The current candidate includes the later combat-hit, contextual route-hint, grounded-defeat, and chroma-fringe fixes, the corrected runtime asset catalog, a materially clearer first-run orientation, and stable meaningful-image geometry across the title, story intros, results, epilogue, pause plaque, and status portrait. Ordinary desktop briefing copy uses an 18px map title, 13px objective and bindings, 12px metadata, and 11px action labels, with stronger 2560 scaling and preserved compact/touch containment. It passed a clean `npm ci`, 397 tests across 36 files, the nested production package, 36 registered browser scenarios, Chromium/Firefox/WebKit smoke, exact 3,660-file `dist`/Pages parity, a zero-vulnerability production audit, 11 focused public/audio tests across three files, a current 3,547-PNG runtime catalog, and the complete alpha-boundary/chroma/spacing validator. The delayed-image contract holds title, episode-intro, and intermission image/content geometry within 0.5px at desktop, high-resolution, portrait, and landscape breakpoints; every changing intro/result image has concise scene-specific alternative text. GitHub Pages then deployed the committed 3,661-file tree including `.nojekyll`; seven sampled live HTML, JS, CSS, transition-art, and audio-manifest byte streams match that tree. The installed browser-game client reaches the Episode 1 story transition and live E1M1 gameplay locally and publicly with complete assets, ready authored audio during gameplay, matching state, and no captured page errors. Use `manifests/release-candidate-evidence.json`, `manifests/release-candidate-preflight.txt`, and `manifests/pages-artifact-sha256.txt` as the candidate header evidence for every human record below.
