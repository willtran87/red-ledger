...
- Previous released software baseline: `be7b49e02a1a0907f409199473d2abee8b50cef9` (`be7b49e`)
- Current automated-release candidate: `3575cdd45eaa7ce880704d393ee895ce6453dbf4` (`3575cdd`)
...
This section supersedes every count, status, and verdict in the archived 2026-07-10 audit below. Commit `be7b49e` remains the previous automated software baseline. Commit `3575cdd45eaa7ce880704d393ee895ce6453dbf4` is the current immutable automated-release candidate. It retains the prior predictive presentation, authored encounter/economy, persistence, mastery, resilience, audio, particle, and local `#playtest` work, then closes the post-release route-state and input gaps: Floor Plan and timed effects are map-scoped and save-restorable; every map has one deliberate timed reward; new secret-clear, Binding Authority, and single-response campaign milestones require valid single-run proof; balance cases delay pickups until after nonlethal damage and add 25% optional pressure; device-family remaps retain cross-device bindings; gamepad navigation repeats and handles disconnects; last-used input drives touch guidance and pointer capture; responsive credential/HUD behavior is verified; and optional semantic sound captions cover gameplay events independently of audible playback. Its exact clean-install preflight is archived in `manifests/release-candidate-preflight.txt`, with structured evidence in `manifests/release-candidate-evidence.json` and a reproducible complete published-file inventory in `manifests/pages-artifact-sha256.txt`.
...
| Status | Meaning |
|---|---|
| **Proven on current candidate** | Direct automated or inspectable evidence passed on immutable commit `3575cdd`. |
...
|---|---|---|---|
| Full release command | **Proven on current candidate** | Clean-install `npm run test:release` passes 314 tests across 32 files, the nested production package, and 32 registered browser scenarios. Exact output is archived with preflight-log SHA-256 `b32a4a8ba6ee98f981ef62e20f9a5b6932662b5343121393665c968dfeab0b7e`. | Automated success does not provide human play, rights, or representative-device signoff. |
...
| Production package and Pages source | **Proven on current candidate** | `dist` contains 3,659 files; Pages contains 3,660 committed files including `.nojekyll`, totaling 74,365,769 raw Git-blob bytes. The reproducible Git-tree manifest hash is `d7ff385a0224b9de92d1b170312ff3d26f995ee7d50cb336cb5ca7dd77638646`, the production audit reports zero vulnerabilities, and deployment run `29513277796` succeeded. Live byte checks match the committed HTML, hashed JS/CSS, audio manifest, E1M1 MP3, and generated status-particle PNG. | Any later `docs` change requires a new committed-tree inventory and live smoke. |
...
Commit `3575cdd45eaa7ce880704d393ee895ce6453dbf4` is promoted as the current **automated-release candidate**. Its clean-install release suite, exact committed Pages inventory, dependency audit, public scan, successful deployment, and live browser smoke pass. It is **not release-final** until the following four human records exist for that same immutable candidate:
...
|---|---|
| Software commit | `3575cdd45eaa7ce880704d393ee895ce6453dbf4` (`3575cdd`) |
| Full release gate | **Pass:** 314 tests / 32 files, nested production package, and 32 browser scenarios after clean `npm ci` |
...
| Performance sample | **Pass:** ready 4,201 ms; 28 hostiles; 535 frames; 18.7 ms mean; 33.3 ms p95; 98 textures |
| Particle stress | **Pass:** 28 baseline draw calls to 30 under stress |
| Production inventory | **Pass:** 3,659 `dist` files; 3,660 committed published files including `.nojekyll`; 74,365,769 raw Git-blob bytes |
| Pages source | **Pass:** reproducible raw committed-tree SHA-256 inventory; manifest hash `d7ff385a0224b9de92d1b170312ff3d26f995ee7d50cb336cb5ca7dd77638646` |
...
| Evidence archive | `manifests/release-candidate-evidence.json`; preflight log SHA-256 `b32a4a8ba6ee98f981ef62e20f9a5b6932662b5343121393665c968dfeab0b7e` |
| Live deployment smoke | **Pass:** Pages run `29513277796`; six exact byte samples; Chromium E1M1 movement/fire with authored audio, no debug API, and zero console/network failures |
