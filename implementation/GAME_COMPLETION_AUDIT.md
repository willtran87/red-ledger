...
- Previous released software baseline: `694b12b24758f3e6c95453e5cf1ab34623fd63dc` (`694b12b`)
- Current automated-release candidate: `be7b49e02a1a0907f409199473d2abee8b50cef9` (`be7b49e`)
...
This section supersedes every count, status, and verdict in the archived 2026-07-10 audit below. Commit `694b12b` remains the previous automated software baseline. Commit `be7b49e02a1a0907f409199473d2abee8b50cef9` is the current immutable automated-release candidate. It adds bounded 35 Hz predictive presentation with a faithful horizontal-only classic mouse preset, cold texture readiness, explicit six-archetype encounter palettes and route-model pars, true secret weapons and clue props, a single-run mastery proof, bounded Regional Director summons including legacy restoration, independent E2M6 pump sectors, a 405-case weapon-aware survival model, and local-only read-only `#playtest` reporting. Its exact clean-install preflight is archived in `manifests/release-candidate-preflight.txt`, with structured evidence in `manifests/release-candidate-evidence.json` and a reproducible complete published-file inventory in `manifests/pages-artifact-sha256.txt`.
...
| Status | Meaning |
|---|---|
| **Proven on current candidate** | Direct automated or inspectable evidence passed on immutable commit `be7b49e`. |
...
|---|---|---|---|
| Full release command | **Proven on current candidate** | Clean-install `npm run test:release` passes 303 tests across 32 files, the nested production package, and 32 registered browser scenarios. Exact output is archived with preflight-log SHA-256 `c4e692e7e422379b6c814b7632afcac446f67a8ff717443df71ec1dc0a93695f`. | Automated success does not provide human play, rights, or representative-device signoff. |
...
| Controls, onboarding, UI, and accessibility | **Proven on current candidate** | Desktop, touch, controller/remapping, responsive layouts, reduced-motion/effects behavior, semantic meters, and entry guidance pass. The 1993 preset retains horizontal mouse yaw while locking pitch; difficulty focus and touch preview are synchronized, slider values are visible and announced, compact zoom layouts reflow, gamepad labels are neutral, MOVE/LOOK/FIRE pads remain labeled, and Reduced Motion retains static muzzle/impact cues without active animation. | Blind E1M1 onboarding with five independent players remains an open human gate. |
...
| Production package and Pages source | **Proven on current candidate** | `dist` contains 3,659 files; Pages contains 3,660 committed files including `.nojekyll`, totaling 74,352,791 raw Git-blob bytes. The reproducible Git-tree manifest hash is `a1a7be0b6101294abe568bcdc52d6378d9a710cec32ae3806344545df3b715ec`, the production audit reports zero vulnerabilities, and deployment run `29506524733` succeeded. Live byte checks match the committed HTML, hashed JS/CSS, audio manifest, E1M1 MP3, and generated status-particle PNG. | Any later `docs` change requires a new committed-tree inventory and live smoke. |
...
Commit `be7b49e02a1a0907f409199473d2abee8b50cef9` is promoted as the current **automated-release candidate**. Its clean-install release suite, exact committed Pages inventory, dependency audit, public scan, successful deployment, and live browser smoke pass. It is **not release-final** until the following four human records exist for that same immutable candidate:
...
|---|---|
| Software commit | `be7b49e02a1a0907f409199473d2abee8b50cef9` (`be7b49e`) |
| Full release gate | **Pass:** 303 tests / 32 files, nested production package, and 32 browser scenarios after clean `npm ci` |
...
| Performance sample | **Pass:** ready 3,638 ms; 28 hostiles; 550 frames; 18.2 ms mean; 33.3 ms p95; 96 textures |
| Particle stress | **Pass:** 28 baseline draw calls to 30 under stress |
| Production inventory | **Pass:** 3,659 `dist` files; 3,660 committed published files including `.nojekyll`; 74,352,791 raw Git-blob bytes |
| Pages source | **Pass:** reproducible raw committed-tree SHA-256 inventory; manifest hash `a1a7be0b6101294abe568bcdc52d6378d9a710cec32ae3806344545df3b715ec` |
...
| Evidence archive | `manifests/release-candidate-evidence.json`; preflight log SHA-256 `c4e692e7e422379b6c814b7632afcac446f67a8ff717443df71ec1dc0a93695f` |
| Live deployment smoke | **Pass:** Pages run `29506524733`; six exact byte samples; Chromium E1M1 movement/fire with authored audio, no debug API, and zero console/network failures |
