...
- Current evidence review: 2026-07-21
- Previous automated-release candidate: `3575cdd45eaa7ce880704d393ee895ce6453dbf4` (`3575cdd`)
- Current automated-release candidate: `ac5e4d6290502366f487f6c4d9cd171126019500` (`ac5e4d6`)
...
This section supersedes every count, status, and verdict in the archived 2026-07-10 audit below. Commit `3575cdd` remains the previous automated software candidate. Commit `ac5e4d6290502366f487f6c4d9cd171126019500` is the current immutable automated-release candidate. It retains the prior predictive presentation, authored encounter/economy, persistence, mastery, resilience, audio, particle, input, accessibility, and local `#playtest` work, then adds the later combat-hit and range corrections, stuck-player route hints, floor-grounded defeated-enemy presentation, and magenta-fringe cleanup. Its runtime catalog has been regenerated against the cleaned PNG bytes and passes its independent freshness check. The exact clean-install preflight is archived in `manifests/release-candidate-preflight.txt`, with structured evidence in `manifests/release-candidate-evidence.json` and a reproducible complete published-file inventory in `manifests/pages-artifact-sha256.txt`.
...
| Status | Meaning |
|---|---|
| **Proven on current candidate** | Direct automated or inspectable evidence passed on immutable commit `ac5e4d6`. |
...
| Performance and lifecycle | **Proven on current candidate** / **Open human gate** | The immutable preflight's SwiftShader active-combat sample delivered 298 frames at 33.7 ms mean / 66.6 ms p95 with 28 hostiles and 98 resident textures; particle stress moved from 28 to 30 draw calls. The E2M8 local report sampled 554 frames at 16.5 ms median / 33.5 ms p95. The local-only `#playtest` report records bounded frame/renderer/audio evidence without persistence or transport, and music streams without long-form buffer decoding. | Representative integrated GPU, discrete GPU, and touch-device targets remain an open human gate. |
...
| Production package and Pages source | **Proven on current candidate** | `dist` contains 3,660 files; Pages contains 3,661 committed files including `.nojekyll`, totaling 73,841,450 raw Git-blob bytes. The reproducible Git-tree manifest hash is `6176cfaa40d9ddde11dcdd3c9492730d1845a089d312d71a937a938c55c19f23`, the production audit reports zero vulnerabilities, and exact-candidate deployment run `29876702723` succeeded. Seven live byte checks match the committed HTML, app/Three.js/CSS bundles, audio manifest, E1M1 MP3, and generated status-particle PNG. | Any later `docs` change requires a new committed-tree inventory and live smoke. |
...
Commit `ac5e4d6290502366f487f6c4d9cd171126019500` is promoted as the current **automated-release candidate**. Its clean-install release suite, exact committed Pages inventory, dependency audit, public scan, runtime-catalog check, complete alpha-boundary art validation, successful exact-candidate deployment, and local/live browser smoke pass. It is **not release-final** until the following four human records exist for that same immutable candidate:
...
|---|---|
| Software commit | `ac5e4d6290502366f487f6c4d9cd171126019500` (`ac5e4d6`) |
...
| Performance sample | **Pass:** ready 5,836 ms; 28 hostiles; 298 frames; 33.7 ms mean; 66.6 ms p95; 98 textures |
| Particle stress | **Pass:** 28 baseline draw calls to 30 under stress |
| Production inventory | **Pass:** 3,660 `dist` files; 3,661 committed published files including `.nojekyll`; 73,841,450 raw Git-blob bytes |
| Pages source | **Pass:** reproducible raw committed-tree SHA-256 inventory; manifest hash `6176cfaa40d9ddde11dcdd3c9492730d1845a089d312d71a937a938c55c19f23` |
...
| Evidence archive | `manifests/release-candidate-evidence.json`; preflight log SHA-256 `ee8913be2fe2166ac87c206e01e249d59752d42c47801ae3c4281929028243b5` |
| Live deployment smoke | **Pass:** Pages run `29876702723`; seven exact byte samples; installed Chromium client reaches E1M1 locally and live with complete assets, authored audio, no captured page errors, and no visually observed magenta fringe |
