...
- Previous automated-release candidate: `2ed9f0f804653c42a26cb4ebfeba9ae84f36f1a0` (`2ed9f0f`)
- Current automated-release candidate: `e64ff6e14db39680691e12cf1a2e50118ef04c95` (`e64ff6e`)
...
This section supersedes every count, status, and verdict in the archived 2026-07-10 audit below. Commit `2ed9f0f` remains the previous automated software candidate. Commit `e64ff6e14db39680691e12cf1a2e50118ef04c95` is the current immutable automated-release candidate. It retains the prior predictive presentation, authored encounter/economy, persistence, mastery, resilience, audio, particle, input, accessibility, combat-hit/range corrections, route hints, grounded defeated-enemy presentation, chroma cleanup, current runtime catalog, clearer first-run orientation, and stable title geometry. It additionally reserves intrinsic geometry for episode intros, intermissions, the epilogue, pause plaque, and status portrait, and gives every changing story/result image scene-specific alternative text. The exact clean-install preflight is archived in `manifests/release-candidate-preflight.txt`, with structured evidence in `manifests/release-candidate-evidence.json` and a reproducible complete published-file inventory in `manifests/pages-artifact-sha256.txt`.
...
| Status | Meaning |
|---|---|
| **Proven on current candidate** | Direct automated or inspectable evidence passed on immutable commit `e64ff6e`. |
...
| Performance and lifecycle | **Proven on current candidate** / **Open human gate** | The immutable preflight's SwiftShader active-combat sample delivered 446 frames at 22.5 ms mean / 33.5 ms p95 with 28 hostiles and 98 resident textures; particle stress moved from 28 to 30 draw calls. The E2M8 local report sampled 604 frames at 16.5 ms median / 33.5 ms p95. The local-only `#playtest` report records bounded frame/renderer/audio evidence without persistence or transport, and music streams without long-form buffer decoding. | Representative integrated GPU, discrete GPU, and touch-device targets remain an open human gate. |
...
| Production package and Pages source | **Proven on current candidate** | `dist` contains 3,660 files; Pages contains 3,661 committed files including `.nojekyll`, totaling 73,843,194 raw Git-blob bytes. The reproducible Git-tree manifest hash is `b3d9bf3e69775e80ec02947d8548e31c0b86b3a8ebd6d0f48bbad95dbe45d350`, the production audit reports zero vulnerabilities, and exact-candidate deployment run `29884808058` succeeded. Seven live byte checks match the committed HTML, app/Three.js/CSS bundles, audio manifest, Episode 1 intro, and Episode 1 intermission art. | Any later `docs` change requires a new committed-tree inventory and live smoke. |
...
Commit `e64ff6e14db39680691e12cf1a2e50118ef04c95` is promoted as the current **automated-release candidate**. Its clean-install release suite, exact committed Pages inventory, dependency audit, public scan, runtime-catalog check, complete alpha-boundary art validation, responsive first-run readability and expanded delayed-image layout checks, successful exact-candidate deployment, and local/live browser smoke pass. It is **not release-final** until the following four human records exist for that same immutable candidate:
...
|---|---|
| Software commit | `e64ff6e14db39680691e12cf1a2e50118ef04c95` (`e64ff6e`) |
...
| Performance sample | **Pass:** ready 3,425 ms; 28 hostiles; 446 frames; 22.5 ms mean; 33.5 ms p95; 98 textures |
| Particle stress | **Pass:** 28 baseline draw calls to 30 under stress |
| Production inventory | **Pass:** 3,660 `dist` files; 3,661 committed published files including `.nojekyll`; 73,843,194 raw Git-blob bytes |
| Pages source | **Pass:** reproducible raw committed-tree SHA-256 inventory; manifest hash `b3d9bf3e69775e80ec02947d8548e31c0b86b3a8ebd6d0f48bbad95dbe45d350` |
...
| Evidence archive | `manifests/release-candidate-evidence.json`; preflight log SHA-256 `0b406222c814e8ddae5d020307f596bbb8644dabb24e991954a26f2d09207d8e` |
| Live deployment smoke | **Pass:** Pages run `29884808058`; seven exact byte samples; installed Chromium client reaches the Episode 1 story transition and live E1M1 gameplay locally and publicly with complete assets, authored gameplay audio, stable contained geometry, and no captured page errors |
