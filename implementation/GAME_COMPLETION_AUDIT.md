...
- Previous automated-release candidate: `e708749138333697a187be091485461f56ec246e` (`e708749`)
- Current automated-release candidate: `2ed9f0f804653c42a26cb4ebfeba9ae84f36f1a0` (`2ed9f0f`)
...
This section supersedes every count, status, and verdict in the archived 2026-07-10 audit below. Commit `e708749` remains the previous automated software candidate. Commit `2ed9f0f804653c42a26cb4ebfeba9ae84f36f1a0` is the current immutable automated-release candidate. It retains the prior predictive presentation, authored encounter/economy, persistence, mastery, resilience, audio, particle, input, accessibility, combat-hit/range corrections, route hints, grounded defeated-enemy presentation, chroma cleanup, current runtime catalog, and clearer first-run orientation. It additionally reserves the title art's intrinsic geometry before a late image decode, eliminating the measured 208.8px menu-action jump while preserving the authored responsive scale. The exact clean-install preflight is archived in `manifests/release-candidate-preflight.txt`, with structured evidence in `manifests/release-candidate-evidence.json` and a reproducible complete published-file inventory in `manifests/pages-artifact-sha256.txt`.
...
| Status | Meaning |
|---|---|
| **Proven on current candidate** | Direct automated or inspectable evidence passed on immutable commit `2ed9f0f`. |
...
|---|---|---|---|
| Full release command | **Proven on current candidate** | Clean-install `npm run test:release` passes 397 tests across 36 files, the nested production package, and 36 registered browser scenarios. Exact output is archived with preflight-log SHA-256 `1035bc1f97ea10d6191a0ccadb1a411c4f47dd1d455c0624c2f9ae151dc77cea`. | Automated success does not provide human play, rights, or representative-device signoff. |
...
| Production package and Pages source | **Proven on current candidate** | `dist` contains 3,660 files; Pages contains 3,661 committed files including `.nojekyll`, totaling 73,842,362 raw Git-blob bytes. The reproducible Git-tree manifest hash is `d286c8ab612aded11db556f87aa8d3a28408f495790525374ffdfca62eef15e3`, the production audit reports zero vulnerabilities, and exact-candidate deployment run `29882893317` succeeded. Seven live byte checks match the committed HTML, app/Three.js/CSS bundles, audio manifest, E1M1 MP3, and generated status-particle PNG. | Any later `docs` change requires a new committed-tree inventory and live smoke. |
...
Commit `2ed9f0f804653c42a26cb4ebfeba9ae84f36f1a0` is promoted as the current **automated-release candidate**. Its clean-install release suite, exact committed Pages inventory, dependency audit, public scan, runtime-catalog check, complete alpha-boundary art validation, responsive first-run readability and delayed-image layout checks, successful exact-candidate deployment, and local/live browser smoke pass. It is **not release-final** until the following four human records exist for that same immutable candidate:
...
|---|---|
| Software commit | `2ed9f0f804653c42a26cb4ebfeba9ae84f36f1a0` (`2ed9f0f`) |
...
| Performance sample | **Pass:** ready 4,608 ms; 28 hostiles; 384 frames; 26.1 ms mean; 50 ms p95; 98 textures |
| Particle stress | **Pass:** 28 baseline draw calls to 30 under stress |
| Production inventory | **Pass:** 3,660 `dist` files; 3,661 committed published files including `.nojekyll`; 73,842,362 raw Git-blob bytes |
| Pages source | **Pass:** reproducible raw committed-tree SHA-256 inventory; manifest hash `d286c8ab612aded11db556f87aa8d3a28408f495790525374ffdfca62eef15e3` |
...
| Evidence archive | `manifests/release-candidate-evidence.json`; preflight log SHA-256 `1035bc1f97ea10d6191a0ccadb1a411c4f47dd1d455c0624c2f9ae151dc77cea` |
| Live deployment smoke | **Pass:** Pages run `29882893317`; seven exact byte samples; installed Chromium client reaches the title and live E1M1 gameplay locally and publicly with complete assets, a ready gameplay audio library, stable contained title geometry, and no captured page errors |
